...
def extract_target_contract_quote(
    chain: pd.DataFrame,
    requested_expiry: str | None,
    requested_strike: float | None,
    option_type: str | None,
) -> tuple[dict[str, Any] | None, str | None, bool, str | None]:
    """Extract the requested execution quote before model-quality quarantine.
    ...
    if not resolved_expiry and "expiry_date" in chain and "expiry_days" in chain:
        ...
        if not expiry_table.empty:
            ...
            resolved_expiry = str(
                expiry_table.loc[
                    expiry_table["expiry_days"] == nearest_days,
                    "expiry_date",
                ].astype(str).min()
            )
            expiry_inferred = True
    if requested_strike is None:
        return None, resolved_expiry, expiry_inferred, None

    requested_strike_value = float(requested_strike)
    target_contract: dict[str, Any] = {
        "requested": {
            "expiry": requested_expiry,
            "strike": requested_strike_value,
    ...
    if candidates.empty or "strike" not in candidates:
        return target_contract, resolved_expiry, expiry_inferred, None
    distances = (pd.to_numeric(candidates["strike"], errors="coerce") - requested_strike_value).abs()
    if not distances.notna().any():
        return target_contract, resolved_expiry, expiry_inferred, None
    chosen = candidates.loc[distances.idxmin()]
    ...
    target_contract.update({
        ...
        "exact_strike_match": abs(float(chosen.get("strike")) - requested_strike_value) < 1e-9,
        "exact_option_type_match": option_type is not None,
    })
    quote_timestamp = matched.get("quote_timestamp")
    observed_at = None if quote_timestamp is None or pd.isna(quote_timestamp) else pd.Timestamp(quote_timestamp).isoformat()
    return target_contract, resolved_expiry, expiry_inferred, observed_at


def target_quote_eligible(target_contract: dict[str, Any] | None) -> bool:
    matched = target_contract.get("matched") if target_contract else None
    bid = matched.get("bid") if matched else None
    ask = matched.get("ask") if matched else None
    quote_timestamp = matched.get("quote_timestamp") if matched else None
    if quote_timestamp is None or pd.isna(quote_timestamp):
        return False
    try:
        numeric_bid = float(bid)
        numeric_ask = float(ask)
        parsed_quote_timestamp = pd.Timestamp(quote_timestamp)
    ...
    return bool(
        ...
        and np.isfinite(numeric_bid) and np.isfinite(numeric_ask)
        and not pd.isna(parsed_quote_timestamp)
        and numeric_bid >= 0.0 and numeric_ask > 0.0
        and numeric_ask >= numeric_bid
    )


def execution_quote_projection(target_contract: dict[str, Any] | None) -> dict[str, Any] | None:
    """Strip quarantined chain analytics down to execution-audit evidence."""

    if target_contract is None:
        return None
    matched = target_contract.get("matched")
    projected_match = None if matched is None else {
        key: matched.get(key)
        for key in ("expiry", "strike", "option_type", "bid", "ask", "quote_timestamp")
    }
    return {
        "requested": dict(target_contract.get("requested") or {}),
        "resolved": dict(target_contract.get("resolved") or {}),
        "matched": projected_match,
        "exact_expiry_match": bool(target_contract.get("exact_expiry_match")),
        "exact_strike_match": bool(target_contract.get("exact_strike_match")),
        "exact_option_type_match": bool(target_contract.get("exact_option_type_match")),
        "execution_audit_only": True,
    }
...
class RealtimePredictor:
    ...
    def predict(
        ...
        *,
        ...
        maximum_future_skew_seconds: float = 3.0,
    ) -> dict[str, Any]:
        started = perf_counter()
        symbol = validate_symbol(symbol)
        ...
        if maximum_snapshot_age_seconds is not None:
            maximum_snapshot_age_seconds = float(maximum_snapshot_age_seconds)
            maximum_future_skew_seconds = float(maximum_future_skew_seconds)
            if not np.isfinite(maximum_snapshot_age_seconds) or not 0 <= maximum_snapshot_age_seconds <= 300:
                raise ValueError("maximum_snapshot_age_seconds must be between zero and 300")
            if not np.isfinite(maximum_future_skew_seconds) or not 0 <= maximum_future_skew_seconds <= 5:
                raise ValueError("maximum_future_skew_seconds must be between zero and five")
        ...
        target_contract, resolved_expiry, expiry_inferred, target_quote_observed_at = extract_target_contract_quote(
            chain,
            expiry,
            strike,
            option_type,
        )
        execution_quote_eligible = target_quote_eligible(target_contract)
        ...
        if maximum_snapshot_age_seconds is not None:
            # Validate point-in-time alignment before the model changes ELO or
            # covariance, not only after returning the prediction to Node.
            observed = target_quote_observed_at or latest_quote_timestamp(chain)
            try:
                issued_time = pd.Timestamp(signal_published_at)
                observed_time = pd.Timestamp(observed) if observed is not None else None
                if issued_time.tzinfo is None or observed_time is None or observed_time.tzinfo is None:
                    raise ValueError("a timezone-aware broker timestamp is required")
                lag = (observed_time - issued_time).total_seconds()
                if not np.isfinite(lag) or lag < -maximum_snapshot_age_seconds or lag > maximum_future_skew_seconds:
                    quarantine_reasons.append("snapshot_outside_requested_time_window")
            except (ValueError, TypeError):
                quarantine_reasons.append("snapshot_timestamp_unavailable")
        ...
        if quarantine_reasons:
            ...
            return json_value({
                ...
                "completed_at": datetime.now(timezone.utc).isoformat(),
                "execution_eligible": False,
                "execution_quote_eligible": execution_quote_eligible,
                "quarantined": True,
                "quarantine_reasons": quarantine_reasons,
                "market_state": asdict(market_state),
                "target_contract": execution_quote_projection(target_contract),
                "contract_assessment": None,
                "events": None,
                "chain_context": None,
                ...
                "provenance": {
                    ...
                    "orders_enabled": False,
                    "target_quote_preserved_before_chain_quarantine": target_contract is not None,
        ...
        if training_day_valid and quality_audit.oi_gross > 0.0:
            runtime.oi_history = (
                *runtime.oi_history,
                quality_audit.oi_gross,
            )[-OI_HISTORY_LIMIT:]
        ...
        if target_contract is not None:
            ...
            contract_assessment = assess_contract(
                ...
                matched,
                exact_match=bool(
                    target_contract["exact_expiry_match"]
                    and target_contract["exact_strike_match"]
                    and target_contract["exact_option_type_match"]
                ),
            ).to_dict()
        execution_eligible = execution_quote_eligible
        ...
        output = {
            ...
            "runtime": {
                ...
                "total_ms": round((perf_counter() - started) * 1000.0, 3),
                "checkpoint": ("async" if use_async else "durable_before_return") if training_day_valid else "none_invalid_training_day",
                "prediction_ran": True, "state_updated": training_day_valid,
            },
            "provenance": {
        ...
        if option_type in {"call", "put"}:
            ...
            for horizon, expectation in result.expectations.items():
                value = assess_contract_value(result, (target_contract or {}).get("matched"),
                                              exact_match=execution_quote_eligible, horizon_minutes=horizon)
                base = value.base_iv_stress
                raw_profit = base.probability_profit_before_fees if base is not None else None
                probabilities[str(horizon)] = {
                    "horizon_minutes": float(horizon),
                    "underlying_favorable_probability": expectation.probability_up if option_type == "call" else 1.0 - expectation.probability_up,
                    "probability_semantics": "delta_gamma_normal_option_pnl_after_spread_before_fees",
                    "profit_probability_status": "available" if raw_profit is not None else "unavailable",
                    "unavailable_reasons": value.reasons if raw_profit is None else (),
                    "net_profit_probability": base.probability_profit_after_costs if base is not None else None,
                    **(self._calibrator.project(symbol, raw_profit) if raw_profit is not None else {
                        "raw_profit_probability": None, "calibrated_profit_probability": None,
                    }),
                }
