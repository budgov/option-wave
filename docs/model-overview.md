...
The realtime worker also maintains a bounded, persistent shadow calibrator.
Verified entry/exit pairs update only a challenger logistic intercept and
slope; duplicate workflow IDs are ignored and parameters are clipped. The
primary Ocean Wave forecast is never changed by a single online outcome.
...
Timestamped earnings, macro-calendar and headline-intensity context are also
risk modifiers. They widen forecast variance and reduce confidence near an
event, but never invent a directional signal from an event label alone.
...
`SchwabHTTPClient` is the read-only adapter for Schwab Trader API option chains
and quotes. It accepts a short-lived OAuth access token at runtime and does not
place orders or store brokerage credentials.
...
keys, orchestration, and result objects. The live listener keeps that thin
Python layer warm in one hidden, bounded JSON-lines worker, so imports and
model initialization are not repeated for every Telegram signal. The worker
uses an LRU symbol cache, compact atomic checkpoints, bounded chain context,
periodic collection, and whole-process recycling after a configured request
count.
