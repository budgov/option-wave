...
export function main(argv = process.argv.slice(2)) {
  ...
  for (const [flag, confirmed] of [["--confirm-claim", true], ["--release-claim", false]]) {
    ...
    const result = settleListenerWatchdogClaim(config, claimId, { confirmed });
    process.stdout.write(`${JSON.stringify(result)}\n`);
    return result;
  }
  const result = reconcileListenerRuntimeState(config, { claimRestart: argv.includes("--claim-restart") });
  process.stdout.write(`${JSON.stringify(result)}\n`);
  return result;
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try {
    ...
    console.error(`Listener watchdog failed: ${safeMessage(error)}`);
    process.exitCode = 1;
  }
}
